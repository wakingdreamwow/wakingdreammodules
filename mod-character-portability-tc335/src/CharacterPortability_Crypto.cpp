...
namespace WCPX::Crypto
{
    std::vector<uint8_t> RandomBytes(size_t n)
    {
        ...
        {
            TC_LOG_ERROR("module", "[WCPX] RAND_bytes failed");
            return {};
        }
        return out;
    }
                           ...
                           uint32_t parallelism)
    {
        ...
        if (rc != ARGON2_OK)
        {
            TC_LOG_ERROR("module", "[WCPX] argon2id_hash_raw failed: {}", argon2_error_message(rc));
            return {};
        }
        return key;
    }
              ...
              std::vector<uint8_t>& sigOut)
    {
        ...
        if (!f)
        {
            TC_LOG_ERROR("module", "[WCPX] cannot open private key at {}", privateKeyPemPath);
            return false;
        }
